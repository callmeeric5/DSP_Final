...
def validate_data(df):
    ...
    for expectation_result in validation_results["results"]:
        if not expectation_result["success"]:
            ...
            if "partial_unexpected_list" in expectation_result["result"]:
                ...
                missing_percent = expectation_result['result'].get('missing_percent', 'N/A')
                unexpected_percent = expectation_result['result'].get('unexpected_percent', 'N/A')
    ...
    filepath_bad = (f"/Users/ericwindsor/Documents/EPITA_ERIC/Data_Scicence_Production"
                    f"/DSP_Final/data_raw/split_data/Bad_Data/{datetime.now()}.csv")
    filepath_clean = (f"/Users/ericwindsor/Documents/EPITA_ERIC/Data_Scicence_Production"
...
def find_column_key(kwargs_dict):
    # Loop through the keys and check for a key containing "column" information
    for key in kwargs_dict.keys():
        if "column" in key.lower():
            return kwargs_dict[key]
    # If no key containing "column" information is found, return None
    return None


def send_teams_alert(errors):
    webhook_url = ("https://epitafr.webhook.office.com/webhookb2/"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Failed to send alert to Teams channel. Status code: {response.status_code}")
...
#         print(f"An error occurred while sending the alert: {e}")
# # # Load the input data
# input_data_df = pd.read_csv("../data_raw/Ingestion/data_5.csv")
# #
# # Validate the data
